...
`nextflow run main.nf -profile conda --vcf ./data/toy.vcf --fam ./data/toy.fam --convert_build --input_build 37 --output_build 38 -resume --results ./convert_build_toy/`
...
`nextflow run main.nf -profile conda --vcf ./covertBuild_toy/files/out.vcf --fam ./covertBuild_toy/converted.fam --convert_build --input_build 38 --output_build 37 -resume --results ./covertBuild2_toy/`
...
- The `snpQTv1.0` block including the "Parameters in effect:"
- A series of processes which comprise from left to right:
	...
	- The name of the modules and the processes for example `process > sample_qc:variant_missingness (1)`
...
In your `./results_toy/` directory you should be able to see as many folders as the number of workflows you chose. Based on the previous examples you should be able to see `./results_toy/qc/`, `./results_toy/pop_strat/` and `./results_toy/gwas/` (as well as `./results_toy/imputation/` if you used the `--impute` workflow explained later).
...
- .html reports summarizing all the "before-and-after the threshold" plots generated in each step, as well as a plot demostrating the number of samples and variants in every step.

Below I show the results of `snpQT` for both the toy and the ALS dataset, using the `--qc`, `--pop_strat` and `--gwas` workflows.
...
!!!Note
	Since we have used `--pop_strat`, the Principal Component Analysis in the Variant QC workflow is run AFTER population stratification. This means that the PCA plots at this stage reflect ethnically homogenous samples. If you didn't use `--pop_strat` then variant QC would still run, producing in the end PCA plots potentially with ethnic outliers.
...
!!!Warning
	`--pre_impute` workflow can only be used along `--qc` and can not be linked to `--gwas`.
...
`nextflow run main.nf -profile docker --vcf imputed.vcf.gz --fam toy.fam --post_impute -resume --results ./results_toy/ --sexcheck false`
