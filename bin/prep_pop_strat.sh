...
awk '{print$2}' missing.bim > plink_5_SNPs.txt 

plink --bfile 1kG_MDS3 --extract plink_5_SNPs.txt --make-bed \
	--out 1kG_MDS3_plink_5 &>/dev/null

# Extract the variants present in 1000 Genomes dataset from the HapMap dataset.
awk '{print$2}' 1kG_MDS3_plink_5.bim > 1kG_MDS3_SNPs.txt 

plink --bfile missing --extract 1kG_MDS3_SNPs.txt --recode --make-bed \
	--out plink_MDS &>/dev/null

# The datasets must have the same build. Change the build 1000 Genomes data
awk '{print$2,$4}' plink_MDS.map > build_map.txt
